...
export class HdBot implements Bot {
	...
	private url: string = ''
	private headers: JsonDoc | undefined
	...
	async initialize() {
		const model = await getMonitModel('hd')
		if (model) {
			this.url = model.url
		}
	...
	async login(): Promise<void> {
		try {
			const response = await this.ax.post(`${this.url}/hismart/login`, this.payload, {
	...
	async getInverter(sitecode: string): Promise<IInverter[]> {
		try {
			const apiUrl = `${this.url}/hismart/monitoring/site/${sitecode}/inverter`
